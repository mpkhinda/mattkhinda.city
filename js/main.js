(function () {
	...
	function buildTile(p) {
		...
		if (p.link_text && p.link_destination) {
			...
			a.innerHTML = `<span class="tile__link-text">${
				escapeHTML(p.link_text)
			}</span> <span class="tile__link-arrow" aria-hidden="true">↗</span>`;
			body.appendChild(a);
		}
	...
	function initLastUpdated() {
		...
		const rawDateStr = document.lastModified;

		// format date
		const formattedDate = new Date(rawDateStr).toLocaleDateString(
			undefined,
			{
				year: "numeric",
				month: "long",
